...
class BASE2(nn.Module):
    ...
    def forward(self, x):
        ...
        layer4_attention = self.cbam_4(layer4_fusion)
        ...
        layer3_attention = self.cbam_3(layer3_fusion)
        ...
        layer2_attention = self.cbam_2(layer2_fusion)
        ...
        layer1_attention = self.cbam_1(layer1_fusion)

        layer4_feature = self.up_4(layer4_attention)
        layer3_feature = self.up_3(layer3_attention)
        layer2_feature = self.up_2(layer2_attention)
        layer1_feature = self.up_1(layer1_attention)

        fusion_feature = torch.cat((layer1_feature, layer2_feature, layer3_feature, layer4_feature), 1)
        fusion_attention = self.cbam_fusion(fusion_feature)

        layer4_predict = self.layer4_predict(layer4_attention)
        layer3_predict = self.layer3_predict(layer3_attention)
        layer2_predict = self.layer2_predict(layer2_attention)
        layer1_predict = self.layer1_predict(layer1_attention)
