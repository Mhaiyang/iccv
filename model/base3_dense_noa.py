"""
 @Time    : 201/23/19 21:06
 @Author  : TaylorMei
 @Email   : mhy845879017@gmail.com
 
 @Project : iccv
 @File    : base3_dense_noa.py
 @Function:
...
class CCL(nn.Module):
    def __init__(self, planes, rate):
        ...
        self.mapping_1 = nn.Conv2d(planes, planes, kernel_size=1, stride=1, padding=0, dilation=1)
        ...
        self.mapping_2 = nn.Conv2d(2 * planes, planes, kernel_size=1, stride=1, padding=0, dilation=1)
        ...
        self.mapping_3 = nn.Conv2d(3 * planes, planes, kernel_size=1, stride=1, padding=0, dilation=1)
        ...
        self.output_conv = nn.Conv2d(4 * planes, planes, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        x1 = x
        mapping_1 = self.mapping_1(x1)
        local_1 = self.local_1(x1)
        context_1 = self.context_1(x1)
        ccl_1 = mapping_1 + (local_1 - context_1)
        ccl_1 = self.bn(ccl_1)
        ccl_1 = self.relu(ccl_1)

        x2 = torch.cat((x1, ccl_1), 1)
        mapping_2 = self.mapping_2(x2)
        local_2 = self.local_2(x2)
        context_2 = self.context_2(x2)
        ccl_2 = mapping_2 + (local_2 - context_2)
        ccl_2 = self.bn(ccl_2)
        ccl_2 = self.relu(ccl_2)

        x3 = torch.cat((x2, ccl_2), 1)
        mapping_3 = self.mapping_3(x3)
        local_3 = self.local_3(x3)
        context_3 = self.context_3(x3)
        ccl_3 = mapping_3 + (local_3 - context_3)
        ccl_3 = self.bn(ccl_3)
        ccl_3 = self.relu(ccl_3)

        x4 = torch.cat((x3, ccl_3), 1)
        output = self.output_conv(x4)
...
# ########################## NETWORK ##############################
###################################################################
class BASE3_DENSE_NOA(nn.Module):
    def __init__(self, backbone_path=None):
        super(BASE3_DENSE_NOA, self).__init__()
        ...
        self.ccl_4 = CCL(512, 2)
        self.ccl_3 = CCL(256, 3)
        self.ccl_2 = CCL(128, 4)
        self.ccl_1 = CCL(64, 5)
