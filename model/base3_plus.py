...
class CCL(nn.Module):
    def __init__(self, in_planes, out_planes, rate):
        ...
        self.output_conv = nn.Conv2d(out_planes * 6, out_planes, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        local_5 = self.local(ccl_4)
        context_5 = self.context(ccl_4)
        ccl_5 = local_5 - context_5
        ccl_5 = self.bn(ccl_5)
        ccl_5 = self.relu(ccl_5)

        local_6 = self.local(ccl_5)
        context_6 = self.context(ccl_5)
        ccl_6 = local_6 - context_6
        ccl_6 = self.bn(ccl_6)
        ccl_6 = self.relu(ccl_6)

        ccl_fusion = torch.cat((ccl_1, ccl_2, ccl_3, ccl_4, ccl_5, ccl_6), 1)
...
# ########################## NETWORK ##############################
###################################################################
class BASE3_PLUS(nn.Module):
    def __init__(self, backbone_path=None):
        super(BASE3_PLUS, self).__init__()
        ...
        self.ccl_4 = CCL(2048, 1024, 2)
        self.ccl_3 = CCL(1024, 512, 3)
        self.ccl_2 = CCL(512, 256, 4)
        self.ccl_1 = CCL(256, 128, 5)

        self.sc_4 = nn.Conv2d(2048, 1024, 1)
        self.sc_3 = nn.Conv2d(1024, 512, 1)
        self.sc_2 = nn.Conv2d(512, 256, 1)
        self.sc_1 = nn.Conv2d(256, 128, 1)
        ...
        self.up_4 = nn.Sequential(nn.ConvTranspose2d(1024, 128, 16, 8, 4), nn.BatchNorm2d(128), nn.ReLU())
        self.up_3 = nn.Sequential(nn.ConvTranspose2d(512, 128, 8, 4, 2), nn.BatchNorm2d(128), nn.ReLU())
        self.up_2 = nn.Sequential(nn.ConvTranspose2d(256, 128, 4, 2, 1), nn.BatchNorm2d(128), nn.ReLU())
        self.up_1 = nn.Sequential(nn.Conv2d(128, 128, 1, 1, 0), nn.BatchNorm2d(128), nn.ReLU())

        self.cbam_4 = CBAM(128)
        self.cbam_3 = CBAM(128)
        self.cbam_2 = CBAM(128)
        self.cbam_1 = CBAM(128)
        self.cbam_fusion = CBAM(512)

        self.layer4_predict = Predict(128)
        self.layer3_predict = Predict(128)
        self.layer2_predict = Predict(128)
        self.layer1_predict = Predict(128)

        self.fusion_predict = nn.Conv2d(512, 1, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        layer4_fusion = self.layer4_fusion(torch.cat((layer4_sc, layer4_ccl), 1))
        layer3_fusion = self.layer3_fusion(torch.cat((layer3_sc, layer3_ccl), 1))
        layer2_fusion = self.layer2_fusion(torch.cat((layer2_sc, layer2_ccl), 1))
        layer1_fusion = self.layer1_fusion(torch.cat((layer1_sc, layer1_ccl), 1))
