...
# ########################## NETWORK ##############################
###################################################################
class BASE4_SP(nn.Module):
    def __init__(self, backbone_path=None):
        super(BASE4_SP, self).__init__()
        ...
        self.layer3_fusion_2 = nn.Sequential(
            nn.Conv2d(512, 512, 1),
            nn.BatchNorm2d(512), nn.ReLU()
        )
        self.layer2_fusion_2 = nn.Sequential(
            nn.Conv2d(256, 256, 1),
            nn.BatchNorm2d(256), nn.ReLU()
        )
        self.layer1_fusion_2 = nn.Sequential(
            nn.Conv2d(128, 128, 1),
            nn.BatchNorm2d(128), nn.ReLU()
        )

        self.up_4 = nn.Sequential(nn.ConvTranspose2d(1024, 64, 16, 8, 4), nn.BatchNorm2d(64), nn.ReLU())
        self.up_3 = nn.Sequential(nn.ConvTranspose2d(512, 64, 8, 4, 2), nn.BatchNorm2d(64), nn.ReLU())
        self.up_2 = nn.Sequential(nn.ConvTranspose2d(256, 64, 4, 2, 1), nn.BatchNorm2d(64), nn.ReLU())
        self.up_1 = nn.Sequential(nn.Conv2d(128, 64, 1, 1, 0), nn.BatchNorm2d(64), nn.ReLU())
        ...
        self.layer4_predict = nn.Conv2d(64, 1, 3, 1, 1)
        self.layer3_predict = nn.Conv2d(64, 1, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        layer3_feature = self.layer3_fusion_2(layer3_feature)
        ...
        layer2_feature = self.layer2_fusion_2(layer2_feature)
        ...
        layer1_feature = self.layer1_fusion_2(layer1_feature)

        layer4_feature_128 = self.up_4(layer4_feature)
        layer4_feature_128 = self.cbam_4(layer4_feature_128)

        layer3_feature_128 = self.up_3(layer3_feature)
        layer3_feature_128 = self.cbam_3(layer3_feature_128)

        layer2_feature_128 = self.up_2(layer2_feature)
        layer2_feature_128 = self.cbam_2(layer2_feature_128)

        layer1_feature_128 = self.up_1(layer1_feature)
        layer1_feature_128 = self.cbam_1(layer1_feature_128)

        fusion_feature = torch.cat((layer1_feature_128, layer2_feature_128, layer3_feature_128, layer4_feature_128), 1)
        fusion_feature = self.cbam_fusion(fusion_feature)

        layer4_predict = self.layer4_predict(layer4_feature_128)
        layer3_predict = self.layer3_predict(layer3_feature_128)
        layer2_predict = self.layer2_predict(layer2_feature_128)
        layer1_predict = self.layer1_predict(layer1_feature_128)
