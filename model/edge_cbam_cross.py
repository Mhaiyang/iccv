...
class EDGE_CBAM_CROSS(nn.Module):
    def __init__(self, backbone_path=None):
        super(EDGE_CBAM_CROSS, self).__init__()
        ...
        self.predict_f0 = Predict(128)

        self.predict_b3 = Predict(512)
        self.predict_b2 = Predict(256)
        self.predict_b1 = Predict(128)
        self.predict_b0 = Predict(128)

        self.predict_edge = nn.Conv2d(64, 1, 3, 1, padding=1)

        self.predict_fb = Fuse()

        self.reduce_dim_1024_512 = nn.Conv2d(1024, 512, 3, padding=1)
        self.reduce_dim_512_256 = nn.Conv2d(512, 256, 3, padding=1)
        self.reduce_dim_256_128 = nn.Conv2d(256, 128, 3, padding=1)
    ...
    def forward(self, x):
        ...
        f3 = torch.cat((f3, e3), 1)
        f3 = self.cbam1024(f3)
        f3 = self.reduce_dim_1024_512(f3)

        f2 = self.f2(f3, layer2)
        f2 = torch.cat((f2, e2), 1)
        f2 = self.cbam512(f2)
        f2 = self.reduce_dim_512_256(f2)

        f1 = self.f1(f2, layer1)
        f1 = torch.cat((f1, e1), 1)
        f1 = self.cbam256(f1)
        f1 = self.reduce_dim_256_128(f1)

        f0 = self.f0(f1, layer0)
        f0 = torch.cat((f0, e0), 1)
        f0 = self.cbam128(f0)

        b3 = self.b3(layer4, layer3)
        b3 = torch.cat((b3, e3), 1)
        b3 = self.cbam1024(b3)
        b3 = self.reduce_dim_1024_512(b3)

        b2 = self.b2(b3, layer2)
        b2 = torch.cat((b2, e2), 1)
        b2 = self.cbam512(b2)
        b2 = self.reduce_dim_512_256(b2)

        b1 = self.b1(b2, layer1)
        b1 = torch.cat((b1, e1), 1)
        b1 = self.cbam256(b1)
        b1 = self.reduce_dim_256_128(b1)

        b0 = self.b0(b1, layer0)
        b0 = torch.cat((b0, e0), 1)
        b0 = self.cbam128(b0)
        ...
        # predict_f3 = F.upsample(predict_f3, size=x.size()[2:], mode='bilinear')
        # predict_f2 = F.upsample(predict_f2, size=x.size()[2:], mode='bilinear')
        # predict_f1 = F.upsample(predict_f1, size=x.size()[2:], mode='bilinear')
        # predict_f0 = F.upsample(predict_f0, size=x.size()[2:], mode='bilinear')
        #
        # predict_b3 = F.upsample(predict_b3, size=x.size()[2:], mode='bilinear')
        # predict_b2 = F.upsample(predict_b2, size=x.size()[2:], mode='bilinear')
        # predict_b1 = F.upsample(predict_b1, size=x.size()[2:], mode='bilinear')
        # predict_b0 = F.upsample(predict_b0, size=x.size()[2:], mode='bilinear')
        #
        # predict_e = F.upsample(predict_e, size=x.size()[2:], mode='bilinear')
        #
        # predict_fb = F.upsample(predict_fb, size=x.size()[2:], mode='bilinear')
