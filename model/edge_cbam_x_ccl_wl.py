...
class EDGE_CBAM_X_CCL(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            return F.sigmoid(predict_f4), F.sigmoid(predict_f3), F.sigmoid(predict_f2), F.sigmoid(predict_f1), \
                   F.sigmoid(predict_b4), F.sigmoid(predict_b3), F.sigmoid(predict_b2), F.sigmoid(predict_b1), \
                   F.sigmoid(predict_e), F.sigmoid(predict_fb)
