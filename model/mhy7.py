...
class Contrast(nn.Module):
    def __init__(self, planes):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(self.inplanes, self.outplanes, kernel_size=3, stride=1, padding=1),
        ...
        self.context_1 = nn.Conv2d(self.outplanes, self.outplanes, kernel_size=3, stride=1, padding=3, dilation=3)

        self.conv2 = nn.Sequential(nn.Conv2d(self.inplanes, self.outplanes, kernel_size=3, stride=1, padding=1),
                                   nn.BatchNorm2d(self.outplanes), nn.ReLU())
        self.local_2 = nn.Conv2d(self.outplanes, self.outplanes, kernel_size=3, stride=1, padding=1, dilation=1)
        self.context_2 = nn.Conv2d(self.outplanes, self.outplanes, kernel_size=3, stride=1, padding=4, dilation=4)

        self.conv3 = nn.Sequential(nn.Conv2d(self.inplanes, self.outplanes, kernel_size=3, stride=1, padding=1),
                                   nn.BatchNorm2d(self.outplanes), nn.ReLU())
        self.local_3 = nn.Conv2d(self.outplanes, self.outplanes, kernel_size=3, stride=1, padding=1, dilation=1)
        self.context_3 = nn.Conv2d(self.outplanes, self.outplanes, kernel_size=3, stride=1, padding=5, dilation=5)

        self.conv4 = nn.Sequential(nn.Conv2d(self.inplanes, self.outplanes, kernel_size=3, stride=1, padding=1),
                                   nn.BatchNorm2d(self.outplanes), nn.ReLU())
        self.local_4 = nn.Conv2d(self.outplanes, self.outplanes, kernel_size=3, stride=1, padding=1, dilation=1)
        self.context_4 = nn.Conv2d(self.outplanes, self.outplanes, kernel_size=3, stride=1, padding=6, dilation=6)
    ...
    def forward(self, x):
        conv1 = self.conv1(x)
        local_1 = self.local_1(conv1)
        context_1 = self.context_1(conv1)
        contrast_1 = local_1 - context_1
        contrast_1 = self.bn(contrast_1)
        contrast_1 = self.relu(contrast_1)

        conv2 = self.conv2(x) + contrast_1
        local_2 = self.local_2(conv2)
        context_2 = self.context_2(conv2)
        contrast_2 = local_2 - context_2
        contrast_2 = self.bn(contrast_2)
        contrast_2 = self.relu(contrast_2)

        conv3 = self.conv3(x) + contrast_2
        local_3 = self.local_3(conv3)
        context_3 = self.context_3(conv3)
        contrast_3 = local_3 - context_3
        contrast_3 = self.bn(contrast_3)
        contrast_3 = self.relu(contrast_3)

        conv4 = self.conv4(x) + contrast_3
        local_4 = self.local_4(conv4)
        context_4 = self.context_4(conv4)
        contrast_4 = local_4 - context_4
        contrast_4 = self.bn(contrast_4)
        contrast_4 = self.relu(contrast_4)

        output = self.cbam(torch.cat((contrast_1, contrast_2, contrast_3, contrast_4), 1))
...
# ########################## NETWORK ##############################
###################################################################
class MHY7(nn.Module):
    def __init__(self, backbone_path=None):
        super(MHY7, self).__init__()
        ...
        self.contrast_4 = Contrast(2048)
        self.contrast_3 = Contrast(1024)
        self.contrast_2 = Contrast(512)
        self.contrast_1 = Contrast(256)
