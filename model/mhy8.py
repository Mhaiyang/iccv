...
class CP(nn.Module):
    def __init__(self, planes, rate):
        ...
        self.cascade = Cascade(planes, rate)
        self.parallel = Parallel(planes)

    def forward(self, x):
        cascade = self.cascade(x)
        parallel = self.parallel(x + cascade)
...
class Parallel(nn.Module):
    def __init__(self, planes):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(self.inplanes, self.outplanes, 3, 1, 1),
                                  nn.BatchNorm2d(self.outplanes), nn.ReLU())

        self.conv2 = nn.Sequential(nn.Conv2d(self.inplanes, self.outplanes, 3, 1, 1),
                                   nn.BatchNorm2d(self.outplanes), nn.ReLU())

        self.conv3 = nn.Sequential(nn.Conv2d(self.inplanes, self.outplanes, 3, 1, 1),
                                   nn.BatchNorm2d(self.outplanes), nn.ReLU())

        self.conv4 = nn.Sequential(nn.Conv2d(self.inplanes, self.outplanes, 3, 1, 1),
                                   nn.BatchNorm2d(self.outplanes), nn.ReLU())

        self.local_1 = nn.Conv2d(self.outplanes, self.outplanes, kernel_size=3, stride=1, padding=1, dilation=1)
        self.context_1 = nn.Conv2d(self.outplanes, self.outplanes, kernel_size=3, stride=1, padding=2, dilation=2)

        self.local_2 = nn.Conv2d(self.outplanes, self.outplanes, kernel_size=3, stride=1, padding=1, dilation=1)
        self.context_2 = nn.Conv2d(self.outplanes, self.outplanes, kernel_size=3, stride=1, padding=4, dilation=4)

        self.local_3 = nn.Conv2d(self.outplanes, self.outplanes, kernel_size=3, stride=1, padding=1, dilation=1)
        self.context_3 = nn.Conv2d(self.outplanes, self.outplanes, kernel_size=3, stride=1, padding=8, dilation=8)

        self.local_4 = nn.Conv2d(self.outplanes, self.outplanes, kernel_size=3, stride=1, padding=1, dilation=1)
        self.context_4 = nn.Conv2d(self.outplanes, self.outplanes, kernel_size=3, stride=1, padding=16, dilation=16)
    ...
    def forward(self, x):
        x1 = self.conv1(x)
        local_1 = self.local_1(x1)
        context_1 = self.context_1(x1)
        ccl_1 = local_1 - context_1
        ccl_1 = self.bn(ccl_1)
        ccl_1 = self.relu(ccl_1)

        x2 = self.conv2(x)
        local_2 = self.local_2(x2)
        context_2 = self.context_2(x2)
        ccl_2 = local_2 - context_2
        ccl_2 = self.bn(ccl_2)
        ccl_2 = self.relu(ccl_2)

        x3 = self.conv3(x)
        local_3 = self.local_3(x3)
        context_3 = self.context_3(x3)
        ccl_3 = local_3 - context_3
        ccl_3 = self.bn(ccl_3)
        ccl_3 = self.relu(ccl_3)

        x4 = self.conv4(x)
        local_4 = self.local_4(x4)
        context_4 = self.context_4(x4)
        ccl_4 = local_4 - context_4
        ccl_4 = self.bn(ccl_4)
        ccl_4 = self.relu(ccl_4)
...
# ########################## NETWORK ##############################
###################################################################
class MHY8(nn.Module):
    def __init__(self, backbone_path=None):
        super(MHY8, self).__init__()
    ...
    def forward(self, x):
        ...
        cp_4 = self.cp_4(layer4)
        cp_3 = self.cp_3(layer3)
        cp_2 = self.cp_2(layer2)
        cp_1 = self.cp_1(layer1)
