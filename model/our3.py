...
# ########################## NETWORK ##############################
###################################################################
class OUR3(nn.Module):
    def __init__(self, backbone_path=None):
        super(OUR3, self).__init__()
        ...
        self.edge_4 = nn.Sequential(nn.ConvTranspose2d(2048, 32, 16, 8, 4), nn.BatchNorm2d(32), nn.ReLU())
        self.edge_3 = nn.Sequential(nn.ConvTranspose2d(1024, 32, 8, 4, 2), nn.BatchNorm2d(32), nn.ReLU())
        self.edge_2 = nn.Sequential(nn.ConvTranspose2d(512, 32, 4, 2, 1), nn.BatchNorm2d(32), nn.ReLU())
        self.edge_1 = nn.Sequential(nn.Conv2d(256, 32, 1, 1, 0), nn.BatchNorm2d(32), nn.ReLU())

        self.edge_cbam = CBAM(128)
        self.edge_predict = nn.Conv2d(128, 1, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        edge_4 = self.edge_4(layer4)
        edge_3 = self.edge_3(layer3)
        edge_2 = self.edge_2(layer2)
        edge_1 = self.edge_1(layer1)

        edge_predict = self.edge_predict(self.edge_cbam(torch.cat((edge_1, edge_2, edge_3, edge_4), 1)))
        ...
        edge_predict = F.upsample(edge_predict, size=x.size()[2:], mode='bilinear', align_corners=True)

        if self.training:
            return layer4_predict, layer3_predict, layer2_predict, layer1_predict, fusion_predict, edge_predict

        return F.sigmoid(layer4_predict), F.sigmoid(layer3_predict), F.sigmoid(layer2_predict), \
               F.sigmoid(layer1_predict), F.sigmoid(fusion_predict), F.sigmoid(edge_predict)
