...
# ########################## NETWORK ##############################
###################################################################
class TAYLOR3(nn.Module):
    def __init__(self, backbone_path=None):
        super(TAYLOR3, self).__init__()
        ...
        self.up_1 = nn.Sequential(nn.Conv2d(256, 64, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU())
    ...
    def forward(self, x):
        ...
        if self.training:
            return layer4_predict, layer3_predict, layer2_predict, layer1_predict, edge_predict

        return F.sigmoid(layer4_predict), F.sigmoid(layer3_predict), F.sigmoid(layer2_predict), \
               F.sigmoid(layer1_predict), F.sigmoid(edge_predict)
